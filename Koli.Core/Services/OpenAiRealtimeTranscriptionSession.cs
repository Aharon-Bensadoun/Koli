...
public sealed class OpenAiRealtimeTranscriptionSession : IAsyncDisposable
{
    ...
    {
        var sessionModel = OpenAiModelProfiles.ResolveRealtimeSessionModel(settings.Model);
        var model = Uri.EscapeDataString(sessionModel);
        ...
        TranscriptionRequestPlan? plan = null)
    {
        ...
        var transcriptionModel = OpenAiModelProfiles.ResolveRealtimeTranscriptionModel(_settings.Model);
        var usePeriodicCommits = periodicBufferCommits
            && OpenAiModelProfiles.IsGptRealtimeWhisperFamily(transcriptionModel);
        var commitInterval = TimeSpan.FromSeconds(Math.Max(2, periodicCommitIntervalSeconds));
        var lastPeriodicCommit = DateTime.UtcNow;
        ...
        try
        {
            await foreach (var chunk in pcm16kChunks.WithCancellation(cancellationToken).ConfigureAwait(false))
            {
                ...
                if (usePeriodicCommits && DateTime.UtcNow - lastPeriodicCommit >= commitInterval)
                {
                    await SendJsonAsync(
                        """{"type":"input_audio_buffer.commit"}""",
                        traceSend,
                        cancellationToken).ConfigureAwait(false);
                    lastPeriodicCommit = DateTime.UtcNow;
                }
            }
    ...
    public static string BuildSessionUpdateJson(AzureOpenAISettings settings, TranscriptionRequestPlan? plan = null)
    {
        ...
        var transcriptionModel = OpenAiModelProfiles.ResolveRealtimeTranscriptionModel(settings.Model);
        ...
        if (!OpenAiModelProfiles.IsGptRealtimeWhisperFamily(transcriptionModel))
        {
