/* ============================================================================
   Koli — Motion Design Video (LIGHT)  |  Deterministic HTML -> MP4 renderer
   Phases:
...
const FINAL    = path.join(OUT, "koli-promo-light.mp4");
...
async function capture() {
  ...
  catch { console.error("[render] puppeteer missing — run `npm install` in motion-video-koli-light/render first."); process.exit(1); }
...
function mux() {
  ...
  if (sources.length === 0) {
    log("No audio files found in motion-video-koli-light/audio — exporting silent final video.");
