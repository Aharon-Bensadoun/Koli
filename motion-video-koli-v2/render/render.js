...
const FINAL    = path.join(OUT, "koli-v2-promo.mp4");
...
async function capture() {
  ...
  catch { console.error("[render] puppeteer missing — run `npm install` in motion-video-koli-v2/render first."); process.exit(1); }
...
function mux() {
  ...
  if (sources.length === 0) {
    log("No audio files found in motion-video-koli-v2/audio — exporting silent final video.");
